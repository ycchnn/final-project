...
{
public:
	TowerCanon(const Point &p) : Tower(p, attack_range(), 120, TowerType::CANON, 420) {}
	/*Bullet *create_bullet(/*Object *target) {
		...
		return new Bullet(p, TowerSetting::tower_bullet_img_path[static_cast<int>(type)], 300, 20, attack_range());
